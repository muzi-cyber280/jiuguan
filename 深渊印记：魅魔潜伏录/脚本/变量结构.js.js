...
export const Schema = z.object({
  ...
  道具商店: z.object({
    商品目录: z.record(z.string(), z.object({ 描述: z.string(), 价格_DP: z.coerce.number().prefault(0) }).prefault({})).prefault({}),
    物品栏: z.record(z.string(), z.object({ 数量: z.coerce.number().transform(v => Math.max(0, v)).prefault(1), 作用: z.string().prefault(''), 类型: z.enum(['消耗品', '耐用品']).prefault('消耗品') }).prefault({})).prefault({}).transform(data => {
      return Object.fromEntries(Object.entries(data).filter(([k, v]) => (v.类型 ?? '消耗品') === '消耗品' ? v.数量 > 0 : true));
    }),
  }).prefault({}),
  任务列表: z.record(z.string(), z.object({ 任务等级: z.enum(['D', 'C', 'B', 'A', 'S']).prefault('D'), 任务描述: z.string().prefault(''), 任务状态: z.enum(['待接取', '进行中', '已完成', '已失败', '已忽略']).prefault('待接取'), 奖励_DP: z.coerce.number().prefault(0), 奖励_SP: z.coerce.number().prefault(0), 奖励_EXP: z.coerce.number().prefault(0), 失败后果: z.string().prefault('无严重后果') }).prefault({})).prefault({}),
  回收站: z.object({
  ...
  NPC状态: z.record(z.string(), z.object({
    在场: z.boolean().prefault(false),
    备注: z.string().prefault(''),
    怀疑度: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(0),
    成瘾度: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(0),
    当前态度: z.string().prefault('中立'),
  }).prefault({})).prefault({}),
  事件日历: z.object({
    创作事件: z.record(z.string(), z.object({
